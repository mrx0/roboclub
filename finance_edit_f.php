	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
			...
			if ($number != 0){
				...
				$old = 'Клиент ['.$arr['client'].']. Сумма ['.$arr['summ'].']. Месяц ['.$arr['month'].']. Год ['.$arr['year'].']. Тип ['.$arr['type'].']. Комментарий ['.$arr['comment'].'].';
			}else{
			...
			//логирование
			AddLog ('0', $_SESSION['id'], $old, 'Отредактирован платёж ['.$_POST['id'].']. ['.date('d.m.y H:i', $time).']. Сумма ['.$_POST['summ'].']. Месяц ['.$_POST['month'].']. Год ['.$_POST['year'].']. Тип ['.$_POST['type'].']. Комментарий ['.$_POST['comment'].'].');
