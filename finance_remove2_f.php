	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
			...
			$query = "INSERT INTO `journal_finance_rem` (
					...
					'{$_POST['summ']}', '{$_POST['filial']}', '{$_SESSION['id']}', '{$_POST['comment']}') ";		
			...
			//логирование
			AddLog ('0', $_SESSION['id'], '', 'Перенесены средства. Сумма ['.$_POST['summ'].']. Клиент ['.$_POST['client'].']. Из месяца/года ['.$_POST['last_month'].'/'.$_POST['last_year'].']. В месяц/год  ['.$_POST['month'].'/'.$_POST['year'].']. Филиал ['.$_POST['filial'].']. Комментарий ['.$_POST['comment'].'].');
